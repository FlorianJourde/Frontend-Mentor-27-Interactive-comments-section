...
export default function DeleteForm({ commentId, onClose, onUpdate }: { commentId: number, onClose: () => void, onUpdate: () => void, isDeleteFormVisible: boolean }) {
    ...
    <>
      <AnimatePresence>
        <motion.div {...ModalAnimation()} className='overlay z-50 fixed top-0 left-0 w-full h-full bg-[hsl(0deg_0%_0%_/_25%)]'>
          <div className={`absolute w-full max-w-[400px] bg-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-2xl p-8 flex flex-col gap-5 delay-500 transition-all}`}>
